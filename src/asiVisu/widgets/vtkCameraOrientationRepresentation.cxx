...
namespace
{
const double xyzBaseColor[3][3] = { { XAXIS_R, XAXIS_G, XAXIS_B }, { YAXIS_R, YAXIS_G, YAXIS_B },
  { ZAXIS_R, ZAXIS_G, ZAXIS_B } };
const double minusXyzTextBgColor[3][3] = { { XAXIS_R, XAXIS_G, XAXIS_B }, { YAXIS_R, YAXIS_G, YAXIS_B },
  { ZAXIS_R, ZAXIS_G, ZAXIS_B } };
...
void vtkCameraOrientationRepresentation::PositionHandles()
{
  ...
  for (int ax = 0, ptId = 7; ax < 3; ++ax)
  {
    for (int dir = 0; dir < 2; ++dir, ++ptId)
    {
      ...
      handleSrc->SetCenter(*(points + static_cast<std::ptrdiff_t>(ptId * 3) + 0),
                           *(points + static_cast<std::ptrdiff_t>(ptId * 3) + 1),
                           *(points + static_cast<std::ptrdiff_t>(ptId * 3) + 2));
    }
  }
...
void vtkCameraOrientationRepresentation::CreateDefaultProperties()
{
  ...
  for (int ax = 0; ax < 3; ++ax)
  {
    for (int dir = 0; dir < 2; ++dir)
    {
      ...
      {
        labelTextProperty->SetBackgroundColor(*(minusXyzTextBgColor[ax] + 0),
                                              *(minusXyzTextBgColor[ax] + 1),
                                              *(minusXyzTextBgColor[ax] + 2));
      }
      else if (dir == to_underlying(HandleDirType::Plus))
      {
        labelTextProperty->SetBackgroundColor(*(xyzBaseColor[ax] + 0),
                                              *(xyzBaseColor[ax] + 1),
                                              *(xyzBaseColor[ax] + 2));
      }
