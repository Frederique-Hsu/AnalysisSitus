...
{
  m_progress.SendLogMessage(LogInfo(Normal) << str);
}
...
// System
System::System(ActAPI_ProgressEntry progress,
...
int System::solve_BFGS(SubSystem* subsys, bool /*isFine*/, bool isRedundantsolving)
{
    ...
    if (debugMode == IterationLevel) {
        ...
        m_progress.SendLogMessage(LogInfo(Normal) << tmp);
    }
    ...
    int& nonredundantconstrNum)
{
    ...
    if (debugMode == Minimal || debugMode == IterationLevel) {
        ...
        m_progress.SendLogMessage(LogInfo(Normal) << "Sketcher::RedundantSolving-%1-\n"
                                                  << solvername);
    }
    ...
    if (res == Success) {
        ...
        if (debugMode == Minimal || debugMode == IterationLevel)
        {
          m_progress.SendLogMessage(LogInfo(Normal) << "Sketcher Redundant solving: %d redundants\n"
                                                    << (int) (redundant.size()));
        }
        ...
        for (int i = conflictGroupsOrig.size() - 1; i >= 0; i--) {
            ...
            for (std::size_t j = 0; j < conflictGroupsOrig[i].size(); j++) {
                if (redundant.count(conflictGroupsOrig[i][j]) > 0) {
                    ...
                    if (debugMode == IterationLevel) {
                        m_progress.SendLogMessage(LogInfo(Normal) << "(Partially) Redundant, Group %d, index %d, Tag: %d\n"
                                                                  << i
                                                                  << (int) (j)
...
void System::clearSubSystems()
{
    ...
    for (auto& doubleptr : subSystems) {
        delete doubleptr;
    }
    for (auto& doubleptr : subSystemsAux) {
        delete doubleptr;
    }
