    ...
    , numpoles(givennumpoles)
{
    ...
    Standard_ASSERT_RAISE (pvec.size() == 2 * numpoles + 1, "pvec.size() == 2 * numpoles + 1");
    Standard_ASSERT_RAISE (factors.size() == numpoles,      "factors.size() == numpoles");
    ...
    : weights(givenweights)
{
    ...
    Standard_ASSERT_RAISE (pvec.size() > 1, "pvec.size() > 1");
    Standard_ASSERT_RAISE (weights.size() == numpoints, "weights.size() == numpoints");
...
ConstraintSlopeAtBSplineKnot::ConstraintSlopeAtBSplineKnot(BSpline& b, Line& l, size_t knotindex)
{
    ...
    Standard_ASSERT_RAISE (numpoles >= 2, "numpoles >= 2");
...
void ConstraintCurveValue::errorgrad(double* err, double* grad, double* param)
{
    ...
    if (this->pcoord() == this->p.x) {  // this constraint is for X projection
      ...
      /*this constraint is neither X nor Y. Nothing to do.*/
    }
}
