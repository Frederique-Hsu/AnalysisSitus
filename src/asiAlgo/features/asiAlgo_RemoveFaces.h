...
#ifndef _asiAlgo_RemoveFaces_h
#define _asiAlgo_RemoveFaces_h
...
//!
//! asiAlgo_RemoveFaces aRF;             // Feature removal algorithm
...
//!
class asiAlgo_RemoveFaces: public BOPAlgo_BuilderShape
{
  // OCCT RTTI
  //DEFINE_STANDARD_RTTI_INLINE(asiAlgo_RemoveFaces, ActAPI_IAlgorithm)

public: //! @name Constructors

  //! Constructor
  //! @param plotter [in] imperative plotter.
  asiAlgo_RemoveFaces(ActAPI_PlotterEntry  plotter  = nullptr)
...
public: //! @name Setting input data for the algorithm
  ...
  //! Calculates the face normal at the given point.
  //! @param face   [in]  the face to calculate normal for.
  //! @param uv     [in]  uv values where to calculate face normal.
  //! @param normal [out] normal vector.
  //! @param p3d    [out] normal vector location.
  //! Return true if normal vector is calulcated, false -- otherwise.
  bool GetFaceNormal(const TopoDS_Face& face,
                     const gp_Pnt2d&    uv,
                     gp_Dir&            normal,
                     gp_Pnt&            p3d);
...
protected: //! @name Fields
  ...
  ActAPI_PlotterEntry myPlotter;          //!< Plotter entry for drawing in the viewer.
};

#endif // _asiAlgo_RemoveFaces_HeaderFile
